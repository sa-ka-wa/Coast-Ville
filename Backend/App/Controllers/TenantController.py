# App/Controllers/TenantController.py
    ...
    def create_tenant():
        ...
        if data.get('unit_id'):
            ...
            if unit and unit.status != 'available':
                return jsonify({'message': 'Unit is not available'}), 400
        ...
        tenant = Tenant(
            ...
            move_in_date=datetime.strptime(data.get('move_in_date'), '%Y-%m-%d').date() if data.get(
                'move_in_date') else None,
            status='active',
